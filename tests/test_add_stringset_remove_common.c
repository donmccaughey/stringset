...
test_add_stringset_remove_common(void)
{
    ...
    int result = stringset_add_stringset_remove_common(set1, set2);
    assert(0 == result);
    
    assert(4 == set1->count);
    
    assert(0 == strcmp("apple", set1->members[0]));
    assert(0 == strcmp("green", set1->members[1]));
    assert(0 == strcmp("strawberry", set1->members[2]));
    assert(0 == strcmp("watermelon", set1->members[3]));
